...
sed -i "s~REP_DATE~$date_stamp~g" $basic_RMD
...
if [[ $flag_prep == "Y" ]]; then
    ...
    for sample_id in "${sample_list[@]}"; do
        ...
        if [[ $sample_id != *"SRR"* ]]; then
            wgs_id=`cat $wgsDB_file | grep $sample_id | awk -F"," '{print $1}'`
            srr_number=`cat $ncbiDB_file | grep $wgs_id | awk -F"," '{print $3}'`
            samn_number=`cat $ncbiDB_file | grep $wgs_id | awk -F"," '{print $4}'`
        else
            wgs_id="NO_ID"
        ...
        sequence_classification=`cat $pipeline_results | awk -F"\t" -v i=$SID 'FNR == i {print $24}' | awk -F";" '{print $NF}'`
        GAMMA_Beta_Lactam_Resistance_Genes=`cat $pipeline_results | awk -F"\t" -v i=$SID 'FNR == i {print $19}'`
        
        # pull validation
        LabValidation=`cat $pipeline_results | awk -F"\t" -v i=$SID 'FNR == i {print $25}'`
