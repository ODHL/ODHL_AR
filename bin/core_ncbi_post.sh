...
upload_dir=$output_dir/tmp/ncbi
...
# pull the ncbi dir 
ncbi_dir=$(dirname "$wgsDB_file")
cachedDB_file="${date_stamp}_upload_db.csv"
...
if [[ "$flag_post" == "Y" ]]; then
    ...
    cp $srrDB_file $ncbi_dir/srr_db_backup.csv

    # copythe original file to the cache
    cp $srrDB_file $cachedDB_file
